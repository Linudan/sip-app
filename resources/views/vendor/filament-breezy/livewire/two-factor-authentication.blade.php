<x-filament::section :aside="true" :heading="__('filament-breezy::default.profile.2fa.title')" :description="__('filament-breezy::default.profile.2fa.description')">
        ...
        <div style="{{ \Illuminate\Support\Arr::toCssStyles([\Filament\Support\get_color_css_variables('danger',shades: [50, 100, 400, 500, 800])]) }}" class="mb-2 p-4 rounded bg-custom-50 dark:bg-custom-400/10 ring-1 ring-custom-100 dark:ring-custom-500/70">
            <div class="flex">
                <div class="flex-shrink-0">
                    @svg('heroicon-s-shield-exclamation', 'w-5 h-5 text-custom-400', ['style' => 'width: 2rem; height: 2rem;'])
                </div>
                <div class="ml-3">
    ...
    @unless ($user->hasEnabledTwoFactor())
        <h3 class="flex items-center gap-2 text-lg font-medium" style="margin-bottom: 0.5rem;">
            @svg('heroicon-o-exclamation-circle', 'w-5', ['style' => 'width: 2rem; height: 2rem;'])
    ...
    @else
        @if ($user->hasConfirmedTwoFactor())
            <h3 class="flex items-center gap-2 text-lg font-medium" style="margin-bottom: 0.5rem;">
                @svg('heroicon-o-shield-check', 'w-5', ['style' => 'width: 2rem; height: 2rem;'])
            ...
            @if ($showRecoveryCodes)
                <div class="px-4 space-y-3" style="margin-bottom: 1rem;">
        ...
        @else
            <h3 class="flex items-center gap-2 text-lg font-medium" style="margin-bottom: 0.5rem;">
                @svg('heroicon-o-question-mark-circle', 'w-5', ['style' => 'width: 2rem; height: 2rem;'])
            ...
            <div class="flex mt-3 space-x-4" style="margin-bottom: 1rem;">
                <div>
